...
`BranchLoadout` is the runtime authority for equipped standard Branch IDs by stable `slot_id`: `standard_slot_1`, `standard_slot_2`, `standard_slot_3`, and `standard_slot_4`. Duplicate standard archetypes are allowed. An initialized slot may explicitly contain `&""` for EMPTY; this differs from an uninitialized slot and must survive MainWorld recreation. `TreeBranchLoadoutController` owns runtime Branch Node creation and destruction only, instantiating `BranchDefinition.branch_scene`; it does not own the authoritative loadout. BranchProgress and `slot_id + branch_id` talent loadouts survive equip/unequip. `BranchLoadout` remains a low-level runtime service and is not preparation-gated. Do not reintroduce static standard `CombatBranch` instances into `tree.tscn`.

Player-facing standard Branch changes are allowed only during Preparation. Preparation occurs before the first Wave of a new run, after the final Wave of each Substage, and after death before restarting the current Substage. During Preparation, WaveDirector is stopped, no enemies are active, CombatBranches are stopped, TREE opens automatically, and standard Branches may be replaced through BranchLoadout. Outside Preparation, TREE standard loadout controls are locked and read-only. All valid registered standard BranchDefinitions are currently available because no standard Branch unlock system exists. Duplicate standard archetypes remain allowed. Candidate preview must show the preserved `slot_id + branch_id` talent build before confirmation.
...
- TREE is the fullscreen Branch/loadout overview. It may replace standard Branches only during Preparation; outside Preparation those controls are locked. It must not mutate progress, buy talents or upgrades, equip Apex Branches, or duplicate gameplay stat calculations.
