...
func _on_quake_telegraph_started(
	...
	colossus_sprite.speed_scale = (
		float(COLOSSUS_QUAKE_IMPACT_FRAME) / telegraph_duration / quake_fps
	)
	colossus_sprite.play()
	...
	var quake_duration: float = (
		float(quake_frame_count) / quake_fps / colossus_sprite.speed_scale
	)
	attack_visual_tween = create_tween()
	attack_visual_tween.tween_interval(quake_duration)
