...
Implementation parent for this checkpoint: `bc8508dacbc3b3bf90264ca61aa314b5c5d741e0` (`Add stage substages and per-enemy wave entries`)

Checkpoint commit: `Add Bark Runner enemy`
...
- Bark Runner is registered after Bark Beetle, but every production Guardian Grove Wave still contains only Bark Beetle.
...
- A mixed `SpawnDirector` fixture spawned Bark Beetle followed by Bark Runner in one ordered batch: two instances of each type, four unique queue keys, and zero tracked enemies after cleanup. Production Wave Resources were not changed.
...
- Bark Runner is registered but is not yet used by a production Wave.
...
- The mixed automated fixture exercises both real enemy scenes, but a production multi-enemy Wave has not yet been visually tested.
...
- 25D / 25E2 — add a deterministic Substage Wave schedule and the first production Bark Beetle + Bark Runner mixed Waves.
...
- Implementation parent for this checkpoint: `bc8508dacbc3b3bf90264ca61aa314b5c5d741e0`.
