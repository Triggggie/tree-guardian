...
The current prototype has one central tree and four standard branch instances: Strength and Blossom on both the left and right sides. The tree also exposes an empty central `AttachmentPoints/Apex` marker for the future legendary Slot 5. Global content and run services are provided by the `GameContent`, `TreeSouls`, and `RunModifiers` autoloads.
...
- Branch category data, Slot 5 rules, and the empty Apex attachment foundation exist, but there is no persistent Branch Seed unlock model, legendary loot-drop flow, or legendary Branch runtime yet.
...
1. Add a persistent model of unlocked Branch Seeds.
2. Add foundational loot-drop events for legendary Branch Seeds.
3. Implement Thorn Crown as the first legendary Apex Branch.
4. Connect an unlocked Thorn Crown to Slot 5.
5. Later create summon/defender runtime support for Spirit Branch.
