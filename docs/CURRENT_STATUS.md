...
Baseline commit: `086ced9cb5fd1feba9ec38dc33fb2a26174db675` (`Migrate branch talents and upgrades to content resources`)
...
This document describes the repository at the baseline commit above. The working tree was clean before this document was created.

## 2. Current Playable Prototype

The main loop is functional:

- Bark Beetles spawn in waves, move toward the tree, attack it, and grant Branch XP plus Forest Essence drops when defeated.
...
The current authored branch content consists of two BranchDefinition Resources, six UpgradeDefinition Resources, one Strength TalentTreeDefinition with three TalentDefinition Resources, and four TreeSoulDefinition Resources. Definitions own stable IDs, presentation text, ordered relationships, and balance values. Runtime branch nodes own current levels, XP, purchases, cooldown state, and combat state.
...
`resources/content_registry.tres` currently registers exactly two branches in UI/gameplay order—Strength then Blossom—and four Tree Souls—Crimson, Azure, Golden, and Verdant. The top-level enemy, stage, and status-effect lists are currently empty.
...
`GameContent` exposes explicit typed access to branches, upgrades, talent trees, talents, Tree Souls, enemies, stages, and waves. Missing registry or unknown IDs safely return empty arrays or `null` as appropriate.
...
- Stage and Wave structure, scoped Wave IDs, enemy/count array consistency, timing and multiplier values;
...
- Enemy, Stage, Wave, and StatusEffect definition types and lookup APIs exist, but no enemies, stages, or status effects are registered. `WaveManager` still preloads the Bark Beetle scene and constructs wave progression from runtime values instead of Stage/Wave Resources.
...
The tracked status documents `docs/CURRENT_STATUS_EN_2026-08-01.md` and `docs/CURRENT_STATUS_CZ_2026-08-01.md` contain the accepted development order, but they describe the older `fcb3536` state. Their completed SOUL-tab, status-panel, rank-notification, and branch-data items must not be repeated. Tasks 14–17 have since completed the current talent/upgrade Resource migration, scoped registry lookup, and detailed validator work.

The nearest still-open roadmap items, retaining that documented order, are:

- finish Tree Soul V1 visual work by adding the visible tree orb and retesting all four Souls;
- extend the three prototype Strength talents into the planned complete talent tree;
- design and implement the currently absent Blossom talent tree;
- migrate Bark Beetle content into `EnemyDefinition`;
- migrate the hardcoded `WaveManager` runtime to `StageDefinition` and `WaveDefinition`.

The existing roadmap places save/load and prestige after those content migrations. They are known gaps, not the immediate next task.
...
- Implementation baseline documented here: `086ced9cb5fd1feba9ec38dc33fb2a26174db675`.
