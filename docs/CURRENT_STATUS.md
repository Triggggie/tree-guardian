...
`BranchSeedDropDefinition` nests a legendary `BranchDefinition` reference and a `0.0` through `1.0` drop chance inside `EnemyDefinition.branch_seed_drops`. Enemy validation rejects null or invalid drops and duplicate drops for the same stable `branch_id`. `ContentValidator` additionally requires every authored drop Branch to be the matching valid legendary definition registered in `ContentRegistry.branches`.

`BranchSeeds` is an autoloaded `BranchSeedService` placed after `GameContent`. It persists only stable legendary Branch IDs in `user://branch_seed_unlocks.cfg`, using ConfigFile version `1` and section `branch_seed_unlocks`. A new unlock is saved immediately; a failed save rolls back the in-memory change, while missing files load as an empty unlock set. Loaded IDs are de-duplicated and empty IDs are ignored.

Enemy defeat processing checks drops in authored order with a service-owned `RandomNumberGenerator`, skips invalid or already-unlocked seeds, and grants at most one new seed per enemy. After a successful save, `branch_seed_unlocked(branch_id)` fires before `branch_seed_dropped(branch_id, enemy_id, world_position)`. The Bark Beetle death path invokes this processing after XP and Forest Essence rewards and before death feedback; Bark Runner inherits the same integration. Cleanup without `die()` grants nothing, and the `is_dying` guard prevents repeated death calls from granting duplicate loot.
...
- Branch category data, Slot 5 rules, the empty Apex attachment, persistent Branch Seed unlock storage, and the legendary drop-processing foundation exist. There is still no production seed drop, legendary Branch runtime, loot UI, physical loot object, or Apex equip flow.
