...
func test_production_registry() -> void:
	...
	expect(items.size() == 2, "Production registry does not contain exactly two items.")
	expect(items[0].item_id == &"living_bark" and items[1].item_id == &"deep_roots", "Production item registry order is invalid.")
...
func test_validator() -> void:
	...
	invalid_registry.items = [null, create_definition(&"invalid_slot", "Invalid Slot", &"canopy")]
