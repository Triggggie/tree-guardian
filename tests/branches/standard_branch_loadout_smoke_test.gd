...
func run_test() -> void:
	...
	expect(not loadout.equip_standard_branch(&"standard_slot_2", &"strength_branch"), "Upper Slot 2 accepted Strength.")
	expect(controller.get_runtime_branch(&"standard_slot_2") == old_slot_2, "Rejected upper Strength changed runtime.")
	expect(loadout.equip_standard_branch(&"standard_slot_2", &"poison_vine"), "Blossom-to-Poison-Vine swap failed.")
	await get_tree().process_frame
	var new_slot_2: CombatBranch = controller.get_runtime_branch(&"standard_slot_2")
	expect(new_slot_2 != old_slot_2 and new_slot_2.branch_id == &"poison_vine" and new_slot_2.facing_side == 0, "Slot 2 runtime swap is invalid.")
	...
	expect(not loadout.equip_standard_branch(&"standard_slot_4", &"strength_branch"), "Upper Slot 4 accepted Strength.")
	expect(loadout.equip_standard_branch(&"standard_slot_4", &"poison_vine"), "Slot 4 Poison Vine equip failed.")
	await get_tree().process_frame
	var strengths: Array[CombatBranch] = [
		controller.get_runtime_branch(&"standard_slot_1"),
		controller.get_runtime_branch(&"standard_slot_3")
	]
	var effect_sets: Dictionary = {}
	for branch in strengths:
		effect_sets[branch.talent_effect_set] = true
	expect(effect_sets.size() == 2, "Two lower Strength copies share TalentEffectSet objects.")
	...
	for branch in strengths: expect(branch.get_upgrade_level(upgrade_id) == 1, "Shared upgrade missed a lower Strength copy.")
	...
	var strength_progress_before_invalid_restore: BranchProgressRecord = progress.get_progress(&"strength_branch")
	expect(loadout.restore_persistence_state({
	...
	}, get_node("/root/BranchSeeds") as BranchSeedService), "Invalid upper Strength restore fixture failed.")
	await get_tree().process_frame
	expect(
		loadout.get_equipped_branch_id(&"standard_slot_1") == &"strength_branch"
		and loadout.get_equipped_branch_id(&"standard_slot_3") == &"strength_branch",
		"Valid lower Strength assignments did not survive restore."
	)
	expect(
		not loadout.is_standard_slot_initialized(&"standard_slot_2")
		and recreated_controller.get_runtime_branch(&"standard_slot_2") == null,
		"Invalid saved upper Strength remained initialized or active."
	)
	expect(
		progress.get_progress(&"strength_branch") == strength_progress_before_invalid_restore
		and strength_progress_before_invalid_restore.branch_level == 2,
		"Rejecting invalid upper Strength erased shared progression."
	)
